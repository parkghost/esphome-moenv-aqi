...
static const int MAX_FUTURE_PUBLISH_TIME_SECONDS = 300;
...
struct Record {
  ...
  bool validate(esphome::ESPTime time, size_t minutes) const {
    if (!time.is_valid() || publish_time.empty()) return false;
    ...
    ESP_LOGV(TAG, "Time difference: %.0f seconds for publish_time: %s", diff_seconds, publish_time.c_str());
    return diff_seconds >= -MAX_FUTURE_PUBLISH_TIME_SECONDS && diff_seconds <= (double)(minutes * 60);
  }
