...
void MoenvAQI::update() {
  ...
  if (site_name_.value() != last_site_name_ && last_site_name_ != "") {
...
bool MoenvAQI::process_response_(Stream &stream, Record &record, int &total) {
  ...
  if (stream.find("\"total\": \"")) {
    ...
    if (len > 0) {
      ...
      ESP_LOGW(TAG, "Could not read total records");
      total = -1;
      return false;
    }
  } else {
    ESP_LOGW(TAG, "Could not find 'total' field");
    total = -1;
    return false;
  }
  ...
  if (next == ']') {
    ESP_LOGW(TAG, "Empty records array, skipping");
    return false;
  }
