...
void MoenvAQI::update() {
  if (site_name_.value() != last_site_name_ || limit_.value() != last_limit_) {
    ESP_LOGD(TAG, "Site name or limit changed, resetting last_successful_offset_");
    last_successful_offset_ = 0;
    last_site_name_ = site_name_.value();
    last_limit_ = limit_.value();
  }
...
bool MoenvAQI::send_request_() {
  ...
  std::string url_base =
      "https://data.moenv.gov.tw/api/v2/aqx_p_432?language=" + language_.value() + "&api_key=" + api_key_.value() + "&limit=" + std::to_string(limit);
...
bool MoenvAQI::process_response_(Stream &stream, Record &record, int &total) {
  ...
  if (stream.find("\"total\": \"")) {
    ...
    if (len > 0) {
      ...
      ESP_LOGW(TAG, "Could not read total records, continue with -1");
      total = -1;
      return false;
    }
  } else {
    ESP_LOGW(TAG, "Could not find 'total' field, continue with -1");
    total = -1;
    return false;
  }
  ...
  do {
    ...
    if (sitename == target_site_name) {
      ESP_LOGI(TAG, "Found target site: %s", target_site_name.c_str());
...
bool MoenvAQI::validate_record_() {
  ...
  return this->data_.validate(now, this->sensor_expiry_.value() / 1000 / 60);
}
...
void MoenvAQI::publish_states_() {
  ...
  if (validate_record_()) {
    if (this->aqi_ != nullptr) this->aqi_->publish_state(this->data_.aqi);
    if (this->so2_ != nullptr) this->so2_->publish_state(this->data_.so2);
    if (this->co_ != nullptr) this->co_->publish_state(this->data_.co);
    if (this->no_ != nullptr) this->no_->publish_state(this->data_.no);
    if (this->wind_speed_ != nullptr) this->wind_speed_->publish_state(this->data_.wind_speed);
    if (this->co_8hr_ != nullptr) this->co_8hr_->publish_state(this->data_.co_8hr);
    if (this->pm2_5_avg_ != nullptr) this->pm2_5_avg_->publish_state(this->data_.pm2_5_avg);
    if (this->so2_avg_ != nullptr) this->so2_avg_->publish_state(this->data_.so2_avg);
    if (this->o3_ != nullptr) this->o3_->publish_state(this->data_.o3);
    if (this->o3_8hr_ != nullptr) this->o3_8hr_->publish_state(this->data_.o3_8hr);
    if (this->pm10_ != nullptr) this->pm10_->publish_state(this->data_.pm10);
    if (this->pm2_5_ != nullptr) this->pm2_5_->publish_state(this->data_.pm2_5);
    if (this->no2_ != nullptr) this->no2_->publish_state(this->data_.no2);
    if (this->nox_ != nullptr) this->nox_->publish_state(this->data_.nox);
    if (this->wind_direc_ != nullptr) this->wind_direc_->publish_state(this->data_.wind_direc);
    if (this->pm10_avg_ != nullptr) this->pm10_avg_->publish_state(this->data_.pm10_avg);
    if (this->site_id_ != nullptr) this->site_id_->publish_state(this->data_.site_id);
    if (this->longitude_ != nullptr) this->longitude_->publish_state(this->data_.longitude);
    if (this->latitude_ != nullptr) this->latitude_->publish_state(this->data_.latitude);
    if (this->current_site_name_ != nullptr) this->current_site_name_->publish_state(this->data_.site_name);
    if (this->county_ != nullptr) this->county_->publish_state(this->data_.county);
    if (this->pollutant_ != nullptr) this->pollutant_->publish_state(this->data_.pollutant);
    if (this->status_ != nullptr) this->status_->publish_state(this->data_.status);
    if (this->publish_time_ != nullptr) this->publish_time_->publish_state(this->data_.publish_time);
  } else {
    if (this->aqi_ != nullptr) this->aqi_->publish_state(NAN);
    if (this->so2_ != nullptr) this->so2_->publish_state(NAN);
    if (this->co_ != nullptr) this->co_->publish_state(NAN);
    if (this->no_ != nullptr) this->no_->publish_state(NAN);
    if (this->wind_speed_ != nullptr) this->wind_speed_->publish_state(NAN);
    if (this->co_8hr_ != nullptr) this->co_8hr_->publish_state(NAN);
    if (this->pm2_5_avg_ != nullptr) this->pm2_5_avg_->publish_state(NAN);
    if (this->so2_avg_ != nullptr) this->so2_avg_->publish_state(NAN);
    if (this->o3_ != nullptr) this->o3_->publish_state(NAN);
    if (this->o3_8hr_ != nullptr) this->o3_8hr_->publish_state(NAN);
    if (this->pm10_ != nullptr) this->pm10_->publish_state(NAN);
    if (this->pm2_5_ != nullptr) this->pm2_5_->publish_state(NAN);
    if (this->no2_ != nullptr) this->no2_->publish_state(NAN);
    if (this->nox_ != nullptr) this->nox_->publish_state(NAN);
    if (this->wind_direc_ != nullptr) this->wind_direc_->publish_state(NAN);
    if (this->pm10_avg_ != nullptr) this->pm10_avg_->publish_state(NAN);
    if (this->pollutant_ != nullptr) this->pollutant_->publish_state("");
    if (this->status_ != nullptr) this->status_->publish_state("");
  }
}
